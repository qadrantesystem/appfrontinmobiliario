...
class PropertyTypesModule {
  ...
  renderTable() {
    if (!this.data.length) return '<div class="empty-state"><p>No hay tipos de inmueble</p></div>';
    return `<table class="data-table"><thead><tr><th>ID</th><th>Nombre</th><th>Activo</th><th>Acciones</th></tr></thead><tbody>${this.data.map(i => `<tr><td>${i.tipo_inmueble_id}</td><td>${i.nombre}</td><td><span class="badge ${i.activo?'badge-success':'badge-danger'}">${i.activo?'Activo':'Inactivo'}</span></td><td><div class="table-actions"><button class="btn-icon" onclick="window.propertyTypesModule.editItem(${i.tipo_inmueble_id})"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button><button class="btn-icon btn-delete" onclick="window.propertyTypesModule.deleteItem(${i.tipo_inmueble_id})"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button></div></td></tr>`).join('')}</tbody></table>`;
  }

  renderPagination() {
    // Paginador siempre visible
    return `<div class="pagination"><button class="btn btn-outline" onclick="window.propertyTypesModule.previousPage()" ${this.pagination.currentPage===1?'disabled':''}>Anterior</button><span>Página ${this.pagination.currentPage} de ${this.pagination.totalPages}</span><button class="btn btn-outline" onclick="window.propertyTypesModule.nextPage()" ${this.pagination.currentPage===this.pagination.totalPages?'disabled':''}>Siguiente</button></div>`;
  }
  ...
  async loadData() {
    const r = await maintenanceService.getTiposInmueblePaginado(this.pagination.currentPage, this.pagination.pageSize, this.searchTerm);
    this.data = r.data;
  ...
  async refreshTable() {
    await this.loadData();
    const t = document.getElementById('propertyTypesTable');
    ...
    if (h) h.textContent = `Administrar tipos de inmueble (${this.pagination.total} total)`;
  ...
  openModal() {
    this.isEditing = false;
    this.editingId = null;
    const m = document.getElementById('propertyTypeModal');
    const f = document.getElementById('propertyTypeForm');
  ...
  async editItem(id) {
    ...
    document.getElementById('propertyTypeModal').style.display = 'flex';
  }
